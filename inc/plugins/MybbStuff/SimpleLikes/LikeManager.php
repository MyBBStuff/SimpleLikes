...
class MybbStuff_SimpleLikes_LikeManager
{
    ...
    public function formatLikes($postLikes, $post)
    {
        ...
        if (!empty($likeArray)) {
            ...
            if (!empty($postLikes[(int) $post['pid']])) {
                $likeString .= ' ' . $this->lang->simplelikes_and . ' <a href="#pid' . $post['pid'] . '" onclick="MyBB.popupWindow(\'' . $this->mybb->settings['bburl'] . '/misc.php?action=post_likes&amp;post_id=' . $post['pid'] . '\', \'buddyList\', 350, 350); return false;">' . (int) count(
